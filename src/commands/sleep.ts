...
export async function sleep(chatId: number, bot: TelegramBot) {
  ...
  if (model && model.sleep) {
    ...
    return bot.sendMessage(
      chatId,
      stringSplit([
        `Ви не отримуєте повідомлення від ${PM} години до ${AM} години`,
        "Якщо Ви хочете змінити цей графік натисніть на кнопки нижче",
      ]),
      {
        reply_markup: {
  ...
  return bot.sendMessage(
    chatId,
    stringSplit([
      "Напишіть час коли Ви не хочете отримувати сповіщення від сервісу",
      "Наприклад: 23-7",
...
export async function catchMsg(
  ...
  return bot.sendMessage(
    chatId,
    `Теперь від ${PM} години до ${AM} години Ви не будете отримувати сповіщення`
  );
}
